...
<body
			...
			<ul class="listaNav"
				...
				<li style="width: 300px; margin-top: 5%;">
					<div class="input-field col s6">
						<input placeholder="Digite o Nome" name="dado" id="first_name"
							type="text" class="validate"></input> <label for="first_name"
				...
				<li>
					<div>
						<select name="cidade">
							<%
								Ilocalizacao lo = new LocalizacaoDAO();
								List<Localizacao> list = lo.selectAll();
								for (Localizacao obl : list) {
							%>
							<option value="<%=obl.getIdCidade()%>"><%=obl.getNomeCidade()%></option>
							<%
								}
							%>
						</select>
	...
	<div style="padding: 10%; padding-top: 2%; padding-bottom: 2%">
		<table class="highlight">
			...
			<tbody>
				<%
					Ilocalizacao locz = new LocalizacaoDAO();
					Iindustria dao = new IndustriaDAO();
					List<Industria> lista = dao.selectAll();
					for (Industria ob : lista) {
				%>
				<tr>

					<td><%=ob.getIdIndustria()%></td>
					<td><%=ob.getNomeIndustria()%></td>
					<td><%=locz.selectLocalizacao(ob.getLocalizacao()).getNomeCidade()%></td>
					<td><a class="waves-effect waves-light btn-large"
						style="background-color: brown"
						href="industriaS?acao=E&dado=<%=ob.getIdIndustria()%>">Excluir</a></td>
